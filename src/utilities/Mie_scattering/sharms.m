...
% 2. Obtain d_sintheta
    legendre_higher = associated_Legendre(l+1,x);
    legendre_higher_1 = legendre_higher(3:end,:);
    legendre_higher_2 = legendre_higher(1:end-2,:);
    plm_sintheta = -1./(2.*ms) .* (legendre_higher_1 + (l-ms+1).*(l-ms+2).*legendre_higher_2);
    plm_sintheta(ms == 0,:) = legendre_total(ms == 0, :) ./ sqrt(1-x.^2);
    plm_sintheta(ms == 0,abs(x)==1) = 0;
    d_sintheta = plm_sintheta .* norm_legendre;
% 3. Obtain d_dtheta
    legendre_higher_1 = legendre_total - legendre_total;
    legendre_higher_1(1:end-1,:) = legendre_total(2:end,:);
    plm_dtheta = legendre_higher_1 + ms.*x.*plm_sintheta;
