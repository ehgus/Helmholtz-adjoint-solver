function set_kernel(h)
    ...
    ROI_size = h.size;
    h.size = size(h.RI);
    h.utility=DERIVE_OPTICAL_TOOL(h,h.use_GPU); % the utility for the space with border
    h.size = ROI_size;
    warning('on','all');
    ...
    if h.acyclic
        if all(h.boundary_thickness_pixel(1:2)==0)
            x=exp(-1i.*pi.*((1:size(h.potential,3))-1)./size(h.potential,3)/2);
            ...
            for j1 = 1:3
                x=single(exp(-1i.*pi.*((1:size(h.potential,j1))-1)./size(h.potential,j1)/2));
