...
%             params.num_scan_per_iteration = 0; % 0 -> every scan is used
        ...
        function [gradient_RI,err]=get_gradiant_static(RI,input_field,target_intensity,forward_solver)
            warning ('off','all');
            forward_solver.set_RI(RI);
            
            [trans_source,~,source_3D]=forward_solver.solve(input_field);
            diff_field=(trans_source-target_intensity);
    ...
    methods
        ...
        function [gradient_RI,err]=get_gradiant(h,RI,input_field,target_intensity)
            ...
            [gradient_RI,err]=h.get_gradiant_static(RI,input_field(:,:,:,scan_list,:),target_intensity,h.forward_solver);
        end

        function [RI]=solve_old(h,input_field,output_field)
            % set parameters
            RI
