...
for div_num = [8,4,2,1]
    ...
    %% Design case: multi-layer simulation with equal thickness

    pixel_period = round(thickness_pixel(2)/div_num);
    ...
    filename = sprintf('optimized double helix mask_Diameter-%.2fum_%d plate_equal_thickness.mat',diameter,div_num);
...
for div_num = [1,2,4,8]
    filename = sprintf('optimized double helix mask_Diameter-%.2fum_%d plate_equal_thickness.mat',diameter,div_num);
    loaded_rst = load(filename, "RI", "field");
    RI_optimized = loaded_rst.RI;
    E_field = loaded_rst.field;
    ...
    fprintf("final cost of %d plate: %g\n\n",div_num,final_cost)
    E_field_list{div_num} = E_field;
end
