...
filter = CyclicConv2Regularizer.conic(0.1/resolution);
...
regularizer_sequence = { ...
    ...
    CyclicConv2Regularizer(filter,'z',@(step) max(ceil((step-15)/10), 0)), ...
    BinaryRegularizer(RI_list(1), RI_list(2), 0.1, 0.1, @(step) max(ceil((step-40)/5), 0)), ...
    MinimumLengthRegularizer(RI_list(1),RI_list(2),1,10,[0.2 0.9],@(step) ceil((step-50)/5) > 0) ...
    InverseBoundRegularizer(RI_list(1), RI_list(2)) ...
};
grad_weight = 0.1;
...
filename = sprintf('optimized single helix mask_Diameter-%.2fum_single plate.mat',diameter);
save_RI(filename, RI_optimized_single, params.resolution, params.wavelength,E_field_list{1});
filename = sprintf('optimized single helix mask_Diameter-%.2fum_double plate.mat',diameter);
