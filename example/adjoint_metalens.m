...
regularizer_sequence = { ...
    ...
    BoundRegularizer(RI_list(1), RI_list(2),@(step) (step <= 35)), ...
    BinaryRegularizer2(RI_list(1), RI_list(2), 0.1, 0.1, @(step) max(0, ceil((step-35)/8))), ...
    BinaryRegularizer(RI_list(1), RI_list(2), 0.1, 0.1, @(step) max(0, ceil((step-35)/8))), ...
    MinimumLengthRegularizer(RI_list(1),RI_list(2),1,10,[0.1 0.9],@(step) ceil((step-45)/5) > 0) ...
    };
grad_weight = 0.2;
